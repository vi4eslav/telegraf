...
func (s *DockerAPP) Add(id string, conf *Config) error {
...
func (s *DockerAPP) Del(id string) {
...
func (s *DockerAPP) Error(err error) {
	s.errors = append(s.errors, err)
}

func (s *DockerAPP) Store(msg Metric) {
	s.metricsCh <- msg
}

func (s *DockerAPP) Run() {
	go s.run()
}

func (s *DockerAPP) run() {
	log.Println("I! Init dockerapp")
	var err error
	if s.ConfigType == "docker_env" {
		...
		log.Println("I! Using k8s api as source for dockerapp")
		...
		log.Panic("E! Unsupported dockerapp")
		return
	}

	if err != nil {
		log.Panicf("E! dockerapp input: %s", err.Error())
		return
	}
...
func (h *DockerAPP) SampleConfig() string {
	return sampleConfig
}

func (h *DockerAPP) Description() string {
	return "Read flattened metrics from docker containers JSON HTTP endpoints"
}

// Gathers data for all containers.
func (s *DockerAPP) Gather(acc telegraf.Accumulator) error {
	s.metricsLock.Lock()
	metrics := s.metrics[:]
...
func init() {
	inputs.Add("dockerapp", func() telegraf.Input {
		dockerapp := NewDockerAPP()
		dockerapp.Run()
		return dockerapp
	})
}
